...
pub fn prompt_flow_integration_test() {
  ...
  let srv = server.new("Integration Test", "1.0.0")
    |> server.add_prompt(prompt, handler)
    |> server.build
  
  // Verify prompt is registered
  srv |> should.not_equal(server.new("Integration Test", "1.0.0") |> server.build)
}
...
pub fn complex_data_integration_test() {
  let complex_prompt_result = mcp.GetPromptResult(
    messages: [
      mcp.PromptMessage(
        role: mcp.User,
        content: mcp.TextPromptContent(mcp.TextContent(
