...
pub fn content_types_test() {
  let text_content =
    mcp.TextContent(type_: "text", text: "Hello, world!", annotations: None)
...
pub fn message_structures_test() {
  let text_content =
    mcp.TextContent(type_: "text", text: "Test message", annotations: None)
...
pub fn initialization_test() {
  let client_info = mcp.Implementation(name: "test_client", version: "1.0.0")
...
pub fn complex_structures_test() {
  let get_prompt_result =
    mcp.GetPromptResult(
      ...
      meta: Some(
        mcp.Meta(progress_token: Some(mcp.ProgressTokenString("test-token"))),
      ),
    )
...
pub fn boundary_conditions_test() {
  ...
  let empty_content =
    mcp.TextContent(type_: "text", text: "", annotations: None)

  empty_content.text |> should.equal("")

  // Test very long content
  let long_text = string.repeat("a", 10_000)
  let long_content =
    mcp.TextContent(type_: "text", text: long_text, annotations: None)
