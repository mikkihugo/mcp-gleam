...
pub fn add_prompt_test() {
  ...
  let handler = fn(_) {
    mcp.GetPromptResult(messages: [], description: None, meta: None)
    |> Ok
  }
...
pub fn add_resource_test() {
  ...
  let handler = fn(_) {
    mcp.ReadResourceResult(contents: [], meta: None)
    |> Ok
  }
...
pub fn add_tool_test() {
  ...
  let handler = fn(_) {
    mcp.CallToolResult(content: [], is_error: Some(False), meta: None)
    |> Ok
  }
...
pub fn server_capabilities_snapshot_test() {
  ...
  let srv =
    server.new("Snapshot Server", "1.0.0")
...
pub fn multiple_prompts_test() {
  let prompt1 =
    mcp.Prompt(name: "p1", description: Some("First"), arguments: None)
  let prompt2 =
    mcp.Prompt(name: "p2", description: Some("Second"), arguments: None)
...
pub fn multiple_resources_test() {
  ...
  let handler = fn(_) { mcp.ReadResourceResult(contents: [], meta: None) |> Ok }

  let srv =
    server.new("Multi Resource", "1.0.0")
...
pub fn edge_cases_test() {
  ...
  srv_empty_version
  |> should.not_equal(server.new("Test", "1.0.0") |> server.build)
}
